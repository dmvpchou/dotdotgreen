// 點點綠 — 示範資料（classic script，非 module，開 file:// 也能直接用）。
...
var DDG = (function () {
  ...
  function brandsAtStation(key) {
    return BRANDS.filter(function (b) { return (b.stations || []).indexOf(key) !== -1; });
  }
  function stationName(key) {
    var s = STATIONS.filter(function (x) { return x.key === key; })[0];
    return s ? s.name : "";
  }
  function brandByName(name) {
    return BRANDS.filter(function (b) { return b.name === name; })[0] || null;
  }
